...
echo "[game-asteroids] driver=$SDL_VIDEODRIVER WAYLAND_DISPLAY=${WAYLAND_DISPLAY:-} XDG_RUNTIME_DIR=${XDG_RUNTIME_DIR:-} uid=$(id -u)" >>"$LOG" 2>&1
cd /usr/share/APPLaunch/apps/game-asteroids
exec /usr/share/APPLaunch/apps/game-asteroids/asteroids "$@" >>"$LOG" 2>&1
EOF
