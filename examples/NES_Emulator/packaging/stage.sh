...
echo "[nes-emulator] driver=$SDL_VIDEODRIVER WAYLAND_DISPLAY=${WAYLAND_DISPLAY:-} XDG_RUNTIME_DIR=${XDG_RUNTIME_DIR:-} uid=$(id -u)" >>"$LOG" 2>&1
...
if [ -n "$ROM" ]; then
    exec "$APP_DIR/nes-emulator" "$ROM" >>"$LOG" 2>&1
fi
exec "$APP_DIR/nes-emulator" >>"$LOG" 2>&1
EOF
