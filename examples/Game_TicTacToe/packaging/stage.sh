...
echo "[game-tictactoe] driver=$SDL_VIDEODRIVER WAYLAND_DISPLAY=${WAYLAND_DISPLAY:-} XDG_RUNTIME_DIR=${XDG_RUNTIME_DIR:-} uid=$(id -u)" >>"$LOG" 2>&1
cd /usr/share/APPLaunch/apps/game-tictactoe
exec /usr/share/APPLaunch/apps/game-tictactoe/tictactoe "$@" >>"$LOG" 2>&1
EOF
